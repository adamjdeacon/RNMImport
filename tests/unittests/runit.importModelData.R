...
test.importModelData <- function()
{
    ...
    dataStatement <- .importNmModData("$DATA data3.dat IGNORE=I IGNORE=(TIME.EQ.1)")
    
    inputStatement <- .importNmModInput("$INPUT AMT TIME DV")
    
    testInput1 <- as.matrix(importModelData(dataStatement, inputStatement, path = testDir))
    ...
    # second test: 
    dataStatement2 <- .importNmModData("$DATA data4.dat")
    inputStatement2 <- .importNmModInput("$INPUT AMT TIME DV")
    ...
    inputStatement3 <- .importNmModInput("$INPUT AMT TIME DV")
    
    testInput3 <- as.matrix( importModelData(dataStatement3, inputStatement3, path = testDir) )
    ...
    # test IGNORE = 'C'
    dataStatement4 <- .importNmModData("$DATA data3.dat IGNORE='I'")
    ...
    dataStatement5 <- .importNmModData("$DATA data3.dat IGNORE=\"I\"")
    testInput5 <-  as.matrix( importModelData(dataStatement5, inputStatement, path = testDir) )
