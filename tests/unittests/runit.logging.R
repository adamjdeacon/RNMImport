...
{
	checkEquals(RNMImport:::availableLogs(), c("stdReport", "detailedReport", "warnings", "highLevelParse", "lowLevelParse"))
	# should throw an exception
	checkException(RNMImport:::setLogConnection("fakeLog", NULL), msg = "Testing error-handling")
	
	# try closing logs that are not open for the correct errors
	# this checks against ticket 0001157 
	
	z <- try(RNMImport:::closeLogConnection("warnings", FALSE))
	...
	z <- try(RNMImport:::closeLogConnection("warnings", TRUE))
