...
test.readNmData <- function()
{
	...
	x.2 <- RNMImport:::readNmData(file.path(testDir, "data3.dat"), ignore = "I")
	...
	# x.3 <- RNMImport:::readNmData(file.path(testDir, "data3"), 
	# 		ignore = "(V2.EQ.12)")
	...
	x.4 <- as.matrix(RNMImport:::readNmData(file.path(testDir, "data3.dat"), ignore = "@"))
	rownames(x.4) <- NULL
