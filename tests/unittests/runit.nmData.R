...
test.nmData.NMBasic <- function()
{
	...
	test1 <- RNMImport:::nmData(run1)
	checkEquals(test1, RNMImport:::nmData(getProblem(run1)), "Extracting from run and problem directly should give same result")
	test2 <- RNMImport:::nmData(run1, dataType = "input")
	...
	test3 <- RNMImport:::nmData(run1, dataType = "output")
	checkTrue(setequal(names(test3), outputColumns), "Checking that all output data is present")
	test4 <- RNMImport:::nmData(prob, returnMode = "DFList")
	...
	checkEquals( test2, RNMImport:::nmData(prob, dataType = c("input", "blah")), msg = " |invalid type 'blah'discarded")
	try.nmData <- try( RNMImport:::nmData( prob, dataType = "out" ) )
	...
	test5 <- RNMImport:::nmData( prob, subset = TRUE )
	checkEquals(test5, RNMImport:::nmData(prob, subset = dataSubset(prob)), 
			msg = " subset = TRUE and subset = dataSubset(prob) are identical")
	
	checkEquals(RNMImport:::nmData(prob, subset = FALSE), RNMImport:::nmData(prob), msg = " subset = FALSE and susbet = NULL are equivalent")
	
	TESTSUBSET <- "ID == 1"
	
	test6.input <- RNMImport:::nmData(prob, dataType = "input", subset = "ID == 1")
	test6.output <- RNMImport:::nmData(prob, dataType = "output", subset = "ID == 1")
	
	test6.inoutlist <- RNMImport:::nmData(prob, returnMode = "DFList", subset = "ID == 1")
	...
	test6.inoutdf  <- RNMImport:::nmData(prob, returnType = "singleDF", subset = "ID == 1")
	
	checkEquals( test6.inoutdf, subset(RNMImport:::nmData(prob), ID == 1 ))
	# check that subset is passed to NMRun method correctly
	
	checkEquals(RNMImport:::nmData(prob, subset = TRUE), RNMImport:::nmData(run1, subset = TRUE), " subset is passed to NMRun method correctly")
...
# SEX.INPUT AGE.INPUT RACE.INPUT HT.INPUT SMOK.INPUT HCTZ.INPUT PROP.INPUT CON.INPUT WT.INPUT TIME.INPUT SECR.INPUT DV.INPUT ID.INPUT OCC.INPUT
	...
	test2298 <- RNMImport:::nmData(	prob )
...
test.nmData.NMSim <- function()
{
	...
	test1 <- RNMImport:::nmData(prob, dataType = "input")
	checkEquals(RNMImport:::nmData(prob, subProblemNum = 2 ), RNMImport:::nmData(getProblem(run2), subProblemNum = 2), 
			"Extracting from run and problem directly should give same result")
	test2 <- RNMImport:::nmData(prob, dataType = "output")
	...
	test3 <- RNMImport:::nmData(prob, subProblemNum = 2:3, returnMode = "DFList") 
	...
	test4 <- RNMImport:::nmData(prob, subProblemNum = 5)
	x <- union( inputColumns, outputColumns	)
	y <- intersect(inputColumns, outputColumns )
	...
	test5 <- RNMImport:::nmData(prob, subProblemNum = 1:2)
	...
	test6 <- RNMImport:::nmData( prob, subset = TRUE )
	checkEquals(test6, RNMImport:::nmData(prob, subset = dataSubset(prob)), 
			msg = " subset = TRUE and subset = dataSubset(prob) are identical")
	...
	TESTSUBSET <- "ID == 1"
#	
	test7.input <- RNMImport:::nmData(prob, dataType = "input", subset = "ID == 1")
	test7.output <- RNMImport:::nmData(prob, dataType = "output", subset = "ID == 1")
#	
	test7.inoutlist <- RNMImport:::nmData(prob, returnMode = "DFList", subset = "ID == 1")
#	
...
#	
	test7.inoutdf  <- RNMImport:::nmData(prob, returnMode = "singleDF", subset = "ID == 1")
#	
	checkEquals( test7.inoutdf, subset(RNMImport:::nmData(prob), ID == 1 ))
...
test.nmDatabyVarType <- function()
{
	...
	test1 <- RNMImport:::nmDatabyVarType(run1, varTypes = "Covariate")
	checkEquals(dim(test1), c(1061, 4), "dimensions of the data are correct")
	checkTrue(setequal(names(test1), c("AGE", "RACE", "SEX", "SMOK")))
	test2 <- RNMImport:::nmDatabyVarType(run1, varTypes = "Parameter")
	checkEquals(nrow(test2), 1061)
	checkTrue(setequal(names(test2), c("CL", "KA", "V")), msg = "correct variables selected")
	test3 <- RNMImport:::nmDatabyVarType(run1, varTypes = "Eta")
	test4 <- RNMImport:::nmDatabyVarType(run1, varTypes = "Lab covariate")
	checkEquals(c(ncol(test3), ncol(test4) ), c(0, 0), "no etas or lab covariates")
	test5 <- RNMImport:::nmDatabyVarType(run1, varTypes = c("Covariate", "Parameter"))
	checkEquals(test5, cbind(test1, test2))
	test6 <- RNMImport:::nmDatabyVarType(run1, varTypes = c("Covariate", "Parameter"), returnMode = "DFList")
	...
	test7 <- RNMImport:::nmDatabyVarType(run2, varTypes = "Covariate", subProblemNum  = 2)
	test7.2 <- test7
