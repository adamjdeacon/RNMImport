...
test.createCategorical.edgeCase <- function()
{
	...
	strangeDf <- data.frame(X = c(1.78, -1.02,  0.74,  1.24, -0.74,  0.59, -0.32,  0.69, -0.14,1.40, 
					rep(0, 100)) )
	newDf <- RNMImport:::addDerivedCategorical(strangeDf, "X", newVar = "X.CUT", binType = "counts" )
...
test.createCategorical <- function()
{
	...
	#Test Exceptions
	checkException(RNMImport:::addDerivedCategorical(testDF, varName = "D"))
	checkException(RNMImport:::addDerivedCategorical(testDF, varName = "B"))
	checkException(RNMImport:::addDerivedCategorical(testDF, varName = "A", binType = "newmethod"))
	
	#Test Different Cuts
	myDF <- data.frame(A = c(10, 20, 50, 100, 150, 200, 250, 300, 400, 500), B = letters[1:10], stringsAsFactors = FALSE)
	testOne <- RNMImport:::addDerivedCategorical(myDF, varName = "A", binType = "unique")
	testTwo <- RNMImport:::addDerivedCategorical(testOne, varName = "A", breaks = 5, newVar = "CutByRange")
	testThree <- RNMImport:::addDerivedCategorical(testTwo, varName = "A", breaks = c(0, 50, 100, 150, 300, 500), newVar = "ExplicitCuts", binType = "explicitcuts")
	testFour <- RNMImport:::addDerivedCategorical(testThree, varName = "A", breaks = c(0, 50, 100, 150, 300, 500), labels = c("BIN1", "BIN2", "BIN3", "BIN4", "BIN5"), 
					newVar = "ExplicitCutsLabels", 	binType = "explicitcuts")
	testFive <-RNMImport:::addDerivedCategorical(testFour, varName = "A", newVar = "LogCuts", binType = "logrange", breaks = 4)
	testSix <- RNMImport:::addDerivedCategorical(testFive, varName = "A", newVar = "EqualCountCuts", binType = "counts", breaks = 2)
	...
	#Check NMBasic
	testBas <- RNMImport:::addDerivedCategorical(prob1, varName = "WT", breaks = 6, dataType = "input")
	checkException(RNMImport:::addDerivedCategorical(prob1, varName = "WT", breaks = 6, dataType = "Intermediate"))
	...
	#Check NMRun
	testRun <- RNMImport:::addDerivedCategorical(run1, varName = "SEX", binType = "unique", newVar = "FactorSEX")
	checkException(RNMImport:::addDerivedCategorical(run1, varName = "SEX",  binType = "unique", dataType = "Intermediate"))
...
test.createCategorical.NMBasicNM7 <- function()
{
	...
	testProb2 <- RNMImport:::addDerivedCategorical(testProb, "DV")
	
	testFrame <- RNMImport:::nmData(testProb)
	expFrame <- RNMImport:::addDerivedCategorical(testFrame, "DV")
...
test.createCategorical.NMSim <- function()
{
	...
	testSim1 <- RNMImport:::addDerivedCategorical(prob2, varName = "DV", breaks = 5, binType = "counts", newVar = "CountsDV")
	checkException(RNMImport:::addDerivedCategorical(prob2, varName = "DV", breaks = 5, binType = "counts", newVar = "CountsDV", dataType = "Intermediate"))
	...
	testSim2 <- RNMImport:::addDerivedCategorical(prob2, varName = "WT", breaks = c(0, 25, 50, 100, 150), binType = "explicitcuts", newVar = "ExplicitWTCut", dataType = "input")
	checkException(RNMImport:::addDerivedCategorical(prob2, varName = "WT", breaks = c(0, 25, 50, 100, 200), binType = "explicitcuts", dataType = "Intermediate"))
...
test.addedData <- function()
{
	...
	testRun2 <- RNMImport:::addDerivedCategorical(run1, varName = "WT", breaks = 6, dataType = "input")
