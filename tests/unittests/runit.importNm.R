...
# SYSTEM
# runit.importNm.R
# Tests importNm, along with RNMImport:::getThetas, getOmegas and getSigmas
...
test.importNm.Basic <- function()
{
	...
	checkEquals( RNMImport:::getThetas(prob1, what = "final"),  expThetas, msg = " |theta final estimates correct")
	...
	checkEquals(expOmega, RNMImport:::getOmegas(prob1, what = "final"), msg = " |omega final estimates imported as expected")
	
	# check that sigma final estimates imported as expected
	
	expSigma <- array(0.0202, c(1,1), dimnames = list("SIGMA1", "SIGMA1"))
	checkEquals(RNMImport:::getSigmas(run1, what = "final"), expSigma, msg = " |sigma final estimates imported as expected")
	
	## check import of data
	
	allData <- RNMImport:::nmData(run1, dataType = c("input", "output"), returnMode ="DFList")
	...
	run1.inData <- RNMImport:::nmData(run1, dataType = "input")
	run2.inData <- RNMImport:::nmData(run2, dataType = "input")
	...
	checkException(RNMImport:::nmData(prob3), msg = " |Output data missing, so there is an incompatibility in data size\n")
	
	# no output tables, so the output data set should be empty
	
	checkEquals(RNMImport:::nmData(prob3, dataType = "output"), data.frame(), msg = " |output data is empty")
	
	checkEquals(list(RNMImport:::getThetas(prob3), RNMImport:::getOmegas(prob3), RNMImport:::getSigmas(prob3)), 
			list(RNMImport:::getThetas(prob1), RNMImport:::getOmegas(prob1), RNMImport:::getSigmas(prob1)), " |parameter estimates still the same, and available")
	
	obj <- RNMImport:::getObjective(run1, addMinInfo = FALSE)
	...
	checkEquals(RNMImport:::nmData(run4, dataType = "output"), data.frame(), msg = " |empty output data" )
	
	# spot checks that other things were imported correctly
	checkEquals(RNMImport:::getSigmas(run4), RNMImport:::getSigmas(run3), msg = " |identical sigmas ")
	checkEquals(RNMImport:::getThetas(run4), RNMImport:::getThetas(run3), msg = " | identical thetas")
	
	checkEquals(RNMImport:::nmData(run4, dataType = "input"), RNMImport:::nmData(run3, dataType = "input"))
	...
	objective <- RNMImport:::getObjective(run5)
	...
	checkEquals( unname(RNMImport:::getThetas(run5, what = "final")), unname(RNMImport:::getThetas(run1, what = "initial")["initial",, drop = TRUE]), msg = " | final values of THETAS same as run 1 initial")
	checkEquals( unname(RNMImport:::getOmegas(run5, what = "final")), unname(RNMImport:::getSigmas(run1, what = "initial")), msg = " | final values of THETAS same as run 1 initial")
...
test.importNm.SimModel <- function()
{
	...
	thetas <- RNMImport:::getThetas(prob, subProblemNum = 1:5)
	omegas <- RNMImport:::getOmegas(prob, subProblemNum = 1:5)
	sigmas <- RNMImport:::getSigmas(prob, subProblemNum = 1:5)
	...
	objectives <- RNMImport:::getObjective(run2, subProblems = 1:5)
	...
	checkEquals(RNMImport:::nmData(run4, dataType = "output"), structure(list(NSIM = structure(integer(0), .Label = c("1", "2", 
											"3", "4", "5"), class = c("ordered", "factor"))), .Names = "NSIM", row.names = integer(0), class = "data.frame"), msg = " |empty output data" )
	
	# spot checks that other things were imported correctly
	checkEquals(RNMImport:::getSigmas(run4), RNMImport:::getSigmas(run2), msg = " |identical sigmas ")
	checkEquals(RNMImport:::getThetas(run4), RNMImport:::getThetas(run2), msg = " | identical thetas")
	
	checkEquals(RNMImport:::nmData(run4, dataType = "input"), RNMImport:::nmData(run2, dataType = "input"))
	
	allData <- RNMImport:::nmData(run2, dataType = c("input", "output"), returnMode ="DFList")
	...
	fullRep <- RNMImport:::importNmReport( "../TestSimRun/TestData1SIM.lst", path = "(internalunit)" )
...
test.importNm.BasicNM7 <- function()
{
	...
	allThetas <- sapply(as.numeric(1:2), function(i) RNMImport:::getThetas(run1, method = i, what = "final"))
	allOmegas <- lapply(as.numeric(1:2), function(i) RNMImport:::getOmegas(run1, method = i, what = "final"))
	allSigmas <- lapply(as.numeric(1:2), function(i) RNMImport:::getSigmas(run1, method = i, what = "final"))
	# FIXME: Theta standard errors not checked!
	thetaStderr <- RNMImport:::getThetas(run1, method = 1, what = "stderrors")
	omegaStderr <- RNMImport:::getOmegas(run1, method = 1, what = "stderrors")
	sigmaStderr <- RNMImport:::getSigmas(run1, method = 1, what = "stderrors") 
		
	covMatrixMeth1 <- RNMImport:::getEstimateCov( run1, method = 1, corMatrix = TRUE )
	covMatrixMeth2 <- RNMImport:::getEstimateCov( run1, method = 2)
	...
	checkEquals( RNMImport:::getObjective(run1, method = 1:2, addMinInfo = FALSE), c(3335.250, 2339.093) )
	
	# all shrinkages

	ETAShrinks <- lapply(as.numeric(1:2), function(i) RNMImport:::getOmegas(run1, method = i, what = "shrinkage"))
	EPSShrinks <- lapply(as.numeric(1:2), function(i) RNMImport:::getSigmas(run1, method = i, what = "shrinkage"))
	...
	checkEquals(RNMImport:::nmData(run2, dataType = "output"), data.frame(), msg = " |empty output data" )
	
	# spot checks that other things were imported correctly
	checkEquals(RNMImport:::getSigmas(run2), RNMImport:::getSigmas(run1), msg = " |identical sigmas ")
	checkEquals(RNMImport:::getThetas(run2), RNMImport:::getThetas(run1), msg = " | identical thetas")
	
	checkEquals(RNMImport:::nmData(run2, dataType = "input"), RNMImport:::nmData(run1, dataType = "input"))
      }
