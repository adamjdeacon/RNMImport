...
\paragraph{} This set of notes is a brief overview of the RNMImport package.  At the moment (version \packVersion), these are quite terse,
...
require(RNMImport)
@
\paragraph{} The main command for importing a NONMEM run is \rfun{importNm}, which works with a control file and a path.  List files
...
<<echo=TRUE, print=FALSE>>=
print(getThetas(prob, stdError=FALSE))
print(getOmegas(prob, stError=FALSE))
print(getThetas(simProblem, stdError=FALSE))
print(getOmegas(simProblem, stdError=FALSE))
@
...
<<echo=TRUE, print=FALSE>>=
print(getThetas(run, stdError = FALSE, problemNum = 1))
print(getOmegas(prob, stError=FALSE, problemNum = 1))
...
data, which has no postfix.  For simulation problems, one can select a vector of subproblems from which to extract the data.  Note that at the moment if more than one subproblem is selected yet the user requests output and input data as a single \code{data.frame}, and error will be generated due to the mismatch in data size.  In future releases, we plan to allow this by repeating the input data as necessary.
