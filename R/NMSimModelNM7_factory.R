...
NMSimModelNM7 <- function(controlStatements, path, reportContents, versionInfo = c("major" = "VII", "minor" = 1))
{
	...
	{
		nDataRows <- max(sapply(outTables, nrow))
	} else {
		nDataRows <- nrow(outTables)
	}
	...
	with(reportContents, 
			{	
				...
				omegaFinal <- if(!is.null(FinalEstimates$OMEGA)) .rearrangeEstimates(FinalEstimates$OMEGA, nSim, numMethods) else list()
