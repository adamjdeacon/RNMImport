...
#' @description Imports one of the new files produced by NONMEM 7: 
#' .phi, .cor, .cov, etc. into a  list of data.frames.
#' @param file Name of the file
#' @param path Path to the file (can be a NONMEM path in round brackets)
#' @param type Type of the file: One of "cov", "ext", or "phi".  
#' "cov" should be used for .cor, .cov or .coi files. If not set, type 
#' will be based on file extension if one of these patterns.
#' @param tableTitles Single logical flag. Are the table titles present in the file?  
#' Should be FALSE, if, for example, one generates an EXT
#' file using the NOTITLE=1 option.  Currently this option ONLY works when type = "ext"
#'
#' @return a list of data.frames holding the content of the individual sub-tables within each file.  
#' Each table will have as an attribute "method", holding the name of the method associated to it, 
#' if it was available (e.g. not if tableTitles = FALSE)
#' If type is "cov", data will be matrices.
...
importNm7Tables <- function(file, path = ".", 
    type = c("cov", "ext", "phi"), tableTitles = TRUE) {
	
    # try to read the file in, capturing the exception if necessary
	filePath <- processPath(path)
    fileContents <- try(readLines(file.path(filePath, file)), silent = TRUE)
	if (inherits(fileContents, "try-error")) RNMImportStop("Unable to load file")
	
    # set type based on file extention if not set
    isType <- logical(3L)
    isType[1L] <- grepl(pattern = "\\.[Cc][Oo][IiRrVv]$", 
        x = file)
    isType[2L] <- grepl(pattern = "\\.[Ee][Xx][Tt]$", 
        x = file)
    isType[3L] <- grepl(pattern = "\\.[Pp][Hh][Ii]$", 
        x = file)
    if (!"type" %in% names(match.call()) && 
        sum(isType) == 1) {
        type <- c("cov", "ext", "phi")[isType]
    } else {
        type <- match.arg(type)
    }
    ...
    if (length(tableBoundaries) == 0) {
        RNMImportStop( "No table delimeters found in the file : not a valid covariance file?" )
	} else {
        if (length(tableBoundaries) == 1) {
            tableList <- list(fileContents[tableBoundaries:length(fileContents)])
        } else {
            tableList <- splitVector(fileContents, tableBoundaries, includeEnd = TRUE)
        }
    }
	if (tableTitles) {
