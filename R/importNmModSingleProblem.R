...
#	browser()
	...
	#	Add a PRIOR section if it exists in the $SUB block
	if(length(prob$Subroutine)>0){
		zz <- 
				regexpr('(PRIOR|PRI) *=.*([[:space:]]|)',prob$Subroutine)
		if(any(zz>0)){
			prob$PRIOR <- 
					strsplit(prob$Subroutine[which(zz>0)],'(PRIOR|PRI)=')[[1]][2]
		}
	}
	# $INPUT statement
